...
void Propagator::registerType(const std::string &name, PropagatorFactory *factory) {
    get_factory()[name] = factory;
}

boost::shared_ptr<Propagator> Propagator::create(const std::string &name) {
    if(get_factory().find(name)==get_factory().end()) {
        std::cerr<<"Not a valid propagator"<<std::endl;
        exit(1);
    }
    return boost::shared_ptr<Propagator>(get_factory()[name]->create());
}
