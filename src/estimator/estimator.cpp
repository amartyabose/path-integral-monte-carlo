...
void Estimator::registerType(const std::string &name, EstimatorFactory *factory) {
    get_factory()[name] = factory;
}

boost::shared_ptr<Estimator> Estimator::create(const std::string &name) {
    if(get_factory().find(name)==get_factory().end()) {
        std::cerr<<"Not a valid estimator: "<<name<<std::endl;
        exit(1);
    }
    return boost::shared_ptr<Estimator>(get_factory()[name]->create());
}
